...
$(document).ready(function () {
    ...
    function updateTableHeaders() {
        ...
        // Natural Disasters
        $("#nav-natural-disasters-tab span").text(`${$("#natural-disasters-table").DataTable().rows().count()}`);


        // Display Table Panel Title with total result count
        $("#disaster_map-footer").empty();
        $("#disaster_map-footer").append(`Results <span class="badge rounded-pill">
    ...
    var epmDisasterTable = createTable("#natural-disasters-table", disasterColumns)
    ...
    const map = L.map('disaster_map', {drawControl: true, fullscreenControl: true,}).setView([-20.917574, 142.702789], 6);
    ...
    map.on(L.Draw.Event.CREATED, function (drawnShape) {
        ...
        // Get the user shape for disaster subscription
        $("#disaster_map_selection").val(JSON.stringify(drawnShape.layer.toGeoJSON())) // Attaches selected data to subscription form
        ...
        if (map.hasLayer(roadHazardApplicationsFeature)) {
            roadHazardApplicationsFeature.eachLayer(function (layer) {
                ...
                if (turf.booleanIntersects(drawnShapeGJ, layer.toGeoJSON())) {
                    
                    // avoid duplicates for roads
                    road_id = layer.options.feature.api_identifier
                    if (road_id_set.has(road_id)){
                        return;
                    }
                    road_id_set.add(road_id)
                    for (var key in layer.options.feature) {
                        if (layer.options.feature.geometry.hasOwnProperty(key)) {
                            console.log(key);
                        }
                    }
        ...
        if (epmGrantedTable.rows().count() + epmApplicationsTable.rows().count() 
            ...
            + cadastreTable.rows().count() + epmDisasterTable.rows().count() > 0) {                   // if any tenement is selected except epm moratorium
            updateTableHeaders();
            $("#table-panel").slideDown();
        } else {                                                    // if no results are found
            $("#disaster_map-footer").empty();
            $("#disaster_map-footer").append(`No Results Found`);
            $("#table-panel").slideUp();
        }
    });
